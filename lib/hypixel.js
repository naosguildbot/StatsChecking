...
// Caching (ToS: "utilize caching to the best of their abilities to reduce
// requests"). Player-facing data changes slowly; these TTLs keep us well
// under the rate limit while still feeling live for personal use.
const cache = new TTLCache();
const UUID_TTL = 60 * 60 * 1000; // usernames -> uuid rarely change: 1h
const PLAYER_TTL = 2 * 60 * 1000; // profile / skyblock: 2 min

// Client-side guard for the 300 requests / 5 minutes default key limit.
// We also trust Hypixel's RateLimit-* response headers below.
